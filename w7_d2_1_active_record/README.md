...
Create a new model called ```User``` with ```first_name``` and ```last_name``` properties that are strings. Look at the output and try to decifer what files it just made for you.
...
Before we can use our associations, we have to set up our database tables. Run:
